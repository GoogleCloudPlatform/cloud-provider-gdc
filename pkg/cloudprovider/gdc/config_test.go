...
func TestReadCloudProviderConfig(t *testing.T) {
	values := map[string]interface{}{
		"project":                  "fake-garden",
		"orgAdminURL":              "https://org-admin-url",
		"systemClusterURL":         "https://system-cluster-url",
		...
		"zones": []*zoneEndpoints{
			{
				...
				InfrastructureAPI: "https://zonal-infra-server-url",
			},
			{
				Name:              "b",
				ManagementAPI:     "https://zonal-management-server-url-1",
				InfrastructureAPI: "https://zonal-infra-server-url-1",
			},
		},
	}
	...
	data := `
		...
		org-admin-url="{{ .Values.orgAdminURL }}"
		infra-cluster-url="{{ .Values.infraClusterURL }}"
		global-management-server-url="{{ .Values.globalMangementServerURL }}"
		is-lancer="{{ .Values.isLancer }}"
		system-cluster-url="{{ .Values.systemClusterURL }}"
		ca="{{ .Values.caData }}"
